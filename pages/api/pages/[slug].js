...
async function updatePage(req, res, slug) {
  try {
    ...
    if (session.user.role === 'ADMIN' || session.user.id === page.createdById) {
      // 管理员和创建者可以直接更新页面
      const updatedPage = await pageOperations.updatePage(page.id, {
        title,
        content,
        description,
...
async function deletePage(req, res, slug) {
  try {
    ...
    // 删除页面及关联的编辑历史
    // Note: In Supabase, we need to handle this differently
    // This will need proper implementation with Supabase RLS policies
    // For now, we'll mark the page as deleted instead of actually deleting it
    await pageOperations.updatePage(page.id, {
      isDeleted: true,
      isPublished: false
    });
